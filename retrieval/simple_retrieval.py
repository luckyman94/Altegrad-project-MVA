...
def retrieve_descriptions(
    ...
    best = sims.argmax(dim=-1).cpu()

    results = []
    for i, test_id in enumerate(test_ids):
        train_id = train_ids[best[i]]
        results.append({
            "ID": test_id,
        ...
        if i < 5:
            print(f"\nTest ID {test_id} → Train ID {train_id}")
            print(train_id2desc[train_id][:150], "...")

    df = pd.DataFrame(results)
    df.to_csv(output_csv, index=False)
    print(f"\nSaved {len(df)} retrieved descriptions to {output_csv}")
